...
function makeBox(color, size, innerWalls) {
  return { id: nextBoxId++, color, x: 0, y: 0, interior: makeInteriorWorld(size, innerWalls) };
}
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (ch === "#") root.walls.add(x + "," + y);
        ...
        const box = makeBox(data.boxColors[ch] || "#888", cfg.size, cfg.walls);
        box.x = x; box.y = y;
        root.boxes.push(box);
        boxByDigit[ch] = box;
      }
